...
J1 = 2*(np.dot(Gx, Gy))
J2 = np.dot(Gx, Gx) - np.dot(Gy, Gy)
J3 = np.dot(Gx, Gx) + np.dot(Gy, Gy)
...
block_coherence = np.sqrt(np.dot(sigma_J1, sigma_J1) + np.dot(sigma_J2, sigma_J2))
block_coherence = np.divide(block_coherence, sigma_J3)

###############try to optimise this part of the code
compare_matrix = sigma_J3 / (16*16) < np.dot(Gth, Gth)
...
############################
connectivity = 8
output = cv2.connectedComponentsWithStats(block_coherence, connectivity)
labels = output[1]
stats = output[2]
